...
export default function ScrollEntry({ onEnter }: { onEnter: () => void }) {
  ...
  useEffect(() => {
    ...
    const handleTouchMove = (e: TouchEvent) => {
      ...
      const deltaY = touchStartY.current - touchY // Positive = swipe up
      
      // Mark as started on first touch move
      if (!hasStarted && Math.abs(deltaY) > 5) setHasStarted(true)
      ...
      progress.set(newProgress)
      touchStartY.current = touchY // Update for next frame
    ...
    >
      ...
      >
        <span className="text-[10px] uppercase tracking-widest font-mono">
          ...
          <span className="md:hidden">Swipe Up to Begin</span>
        </span>
        <motion.div
          animate={{ y: [0, 5, 0] }}
