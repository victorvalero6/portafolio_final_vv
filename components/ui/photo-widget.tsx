...
export function PhotoWidget({
  ...
  useEffect(() => {
    const loadPhotos = async () => {
      ...
      // Use fetched photos or fallback, but don't shuffle yet to avoid hydration mismatch
      const photosToUse = fetchedPhotos.length > 0 ? fetchedPhotos : fallbackPhotos
  ...
  // Shuffle photos after initial render to avoid hydration mismatch
  useEffect(() => {
    if (photos.length > 0) {
      // Only shuffle on client side after mount
      setPhotos(prev => shuffleArray([...prev]))
    }
  }, [albumKey]) // Re-shuffle only when album changes
