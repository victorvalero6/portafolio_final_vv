...
export function ProjectsGrid() {
  ...
  useEffect(() => {
    ...
    const loadProjects = async () => {
      ...
      try {
        // Build column selection based on language
        const titleCol = `title_${language}`
        const summaryCol = `summary_${language}`
        ...
        const mapped = rows.map((row) => {
          ...
          const tags =
            project.project_technologies?.flatMap((rel: any) => {
              const tech = rel.technologies
              if (!tech) return []
              if (Array.isArray(tech)) {
                return tech.map((item: any) => item?.name).filter(Boolean) as string[]
              }
              return tech.name ? [tech.name] : []
            }) ?? []

          // Get title and summary based on current language, fallback to default
          const getLocalizedField = (fieldPrefix: string) => {
            const langField = project[`${fieldPrefix}_${language}`]
            const enField = project[`${fieldPrefix}_en`]
            const defaultField = project[fieldPrefix]
          ...
          return {
            ...
            summary: getLocalizedField('summary'),
            image: project.featured_image_url,
            imageAlt: project.featured_image_alt ?? getLocalizedField('title'),
